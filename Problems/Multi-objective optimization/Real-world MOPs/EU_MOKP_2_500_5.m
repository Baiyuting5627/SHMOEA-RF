classdef EU_MOKP_2_500_5 < PROBLEM
...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)                           
             ...
             ratio_randprofit = 0.05;                                       % 对应Item的Profit随机扰动比例（前10%的变量进行扰动）
             P_bak = obj.P;                                                % 原始价值向量
             ...
             min_threshold = 1e-10;
             log_value =  log(max(total_value,min_threshold));
             PopObj = repmat(obj.N*obj.D, size(PopDec, 1), 2)-log_value;     %100*2 将目标值从最大值基准中扣除，转化为一个极小化问题
        end
       ...
       function R = GetOptimum(obj,~)
           %data=load('D:\白雨婷\PlatEMO-master-bai\PlatEMO\Real_PF\U_PF_2_1.mat');% 2-250
           data=load('D:\白雨婷\PlatEMO_latest\PlatEMO-master\PlatEMO\Real_PF\Re_U_PF_2_1.mat');% 2-250
           %R=data.PS;          %二次前沿
           R=data.PS.objs;    %普通前沿
