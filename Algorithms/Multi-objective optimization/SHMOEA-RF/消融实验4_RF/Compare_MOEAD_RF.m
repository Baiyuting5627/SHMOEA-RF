...
%--------------------------------------------------------------------------
    ...
    methods
        function main(Algorithm,Problem)
            ...
            num_selet_pop_first = 100;   % 从当前种群中初次选择用于鲁棒平均多次采样评估并更新外部集的(单次评估)解的数量
            num_selet_pop_second = 50;  % 从初次选择的解中，根据RF预测后的函数值进一步选择用于鲁棒平均多次采样评估并更新外部集的(单次评估)解的数量
            ...
            num_pop_elite = 30;    % 单个加权子问题关联最好解的数量      
            ...
            % for i = 1:Problem.N
            %     Population(i) = Greedy_Repair_Scalar(WV(i,:),Problem, Population(i));      % 修复每个解（使用对应的权重向量）
            % end
            %[Population] = Greedy_Repair_Scalar(WV,Problem,Population);            % 随机初始种群进行启发式修正(单次评估的函数值)
            ...
            NPOP = Population(FrontNo == 1);   % 将第一层的非支配解放入外部集中
            gen = 1;                           % 初始化种群的迭代次数为1
          %% Optimization
            while Algorithm.NotTerminated(NPOP)              
                     ...
                     for i = 1 : size(weights,1)          % 遍历子问题下的每一个权重            
                         P = weights(i,:);                % 获取当前子问题对应的第i个权重值     
                         g = sum(Population.objs.*P,2);   % 根据加权和计算当前种群在该权重下的聚合函数(eg.,100*1 的向量）
                         [~, indices] = sort(sum(g, 2));  % 将种群加权聚合函数按照从小到大的顺序进行排列
                         selected_indices = indices(1: num_pop_elite);                 % 根据排序，提取前num_pop_elite个最小聚合函数值对应的索引
                         pop_weight = Population(selected_indices);                    % 提取种群中在当前权重下，具有最小聚合函数值的num_pop_elite个候选解                        
                         Offspring = OperatorGA(Problem,pop_weight);                   % 候选解进行交叉变异,生成后代个体
                         %Offspring = Greedy_Repair_Scalar(P,Problem,Offspring);  % 后代个体基于当前权重进行启发式修正(单次评估的函数值)                                            
                         Offspring = Greedy_Repair_Nonscalar(Problem, Offspring); 
                         g_old = g(selected_indices);       % 提取num_pop_elite个候选解的聚合函数值
                         g_new = sum(Offspring.objs.*P,2);  % 计算后代个体在当前权重下的加权聚合函数值
                         ...
                         POP = [POP,Offspring];                   % 将修正后单次评估的后代个体放入POP,用于后续初次选择解
                     end                                          % 遍历5个权重，共生成100个后代解放入POP      
                     POP = Select_Individuals(POP, Problem.N);    % POP按照分层排序保留解，保证始终存储种群大小数量的好解，用于后续初步选择好解  
                     if mod(gen,num_beta) == 0                    % 当迭代进化次数gen为20的倍数时
                           Selected_Pop_first = Select_Individuals(POP , num_selet_pop_first);    % 按照层级和拥挤距离优先选择，计算分层排序保留第一层的解，若数量不足再计算拥挤距离选择剩余解
