classdef Compare_NSGAII_MOEAD_2 < ALGORITHM
% <2002> <multi> <real/integer/label/binary/permutation> <constrained/none>
% Nondominated sorting genetic algorithm II ,
% 改进 NSGAII + 动态 MOEAD协同进化，初步选解，只是不加RF，其他都一样，贪婪式约束修正
               ...
               %% 两种方式生成后代新解
                if mod(gen, 2 * num_beta) < num_beta
                   ...
                   %%  MOEA/D 子问题分解方法生成新解，eg.,gen = 20-39,60-79,100-119,...            
                    ...
                    for i = 1:size(Weights, 1)                                                    % 遍历子问题下的每一个权重    
                        ...
                        %Offspring = Greedy_Repair_Scalar(weight, Problem, Offspring);    % 后代个体基于当前权重进行启发式修正(单次评估的函数值)                                                     
                ...
                if mod(gen, num_beta) == 0                                                                          % 当迭代进化次数gen为20的倍数时进行初步选解             
                   %% 初步筛选阶段
                    ...
                    NPOP = [NPOP, Selected_Pop_first];                                                % 将鲁棒多次评估后的解加入初次外部集TPOP
                
                    if size(NPOP,1)>=num_select_archive
                       Population = Replace_Pop(NPOP, Population, num_select_archive);   % 从外部集任选num_select_archive个解，更新当前进化种群中的任意num_select_archive个解 
                    end
                    [FrontNo_moead,~] = NDSort(NPOP.objs,inf);                        % 对外部集进行非支配排序，确保外部集中始终存储的是鲁棒多次平均采样评估的非支配解   
                    NPOP = NPOP(FrontNo_moead == 1);           % 将第一层的非支配解放入外部集中
