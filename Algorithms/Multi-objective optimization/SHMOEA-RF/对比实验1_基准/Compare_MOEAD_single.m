classdef Compare_MOEAD_single < ALGORITHM
    ...
    methods
        function main(Algorithm,Problem)
            ...
            A = Multiple_Sampling(Problem,A,num_Samples);                                          % 外部集多次评估;
            gen = 1;
            %% Optimization
            while Algorithm.NotTerminated(A)
                for i = 1 : Problem.N                                                      % 遍历种群中的每个解
                    ...
                    %[Offspring] = Greedy_Repair_Scalar(P(1:2),Problem, Offspring);   
                ...
                gen = gen + 1;
                 %% 输出最终种群（使用多次评估结果）
                % if flag == 0 && Problem.FE >= Problem.maxFE - num_Samples*length(A) 
                %     A = Multiple_Sampling(Problem, A, num_Samples); 
